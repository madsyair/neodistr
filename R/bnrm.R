...
#'   \code{\link[brms::brmsformula]{brmsformula}}, or \code{\link[brms::mvbrmsformula]{mvbrmsformula}} (or one that can
...
#'   \code{\link[brms::set_prior]{set_prior}} or related functions and combined using the
...
#' @param save_pars An object generated by \code{\link[brms::save_pars]{save_pars}} controlling
...
#'   \code{\link[brms::hypothesis]{hypothesis}}. Please note that improper priors are not sampled,
#'   including the default improper priors used by \code{brm}. See
#'   \code{\link[brms::set_prior]{set_prior}} on how to set (proper) priors. Please also note
#'   that prior draws for the overall intercept are not obtained by default
#'   for technical reasons. See \code{\link[brms::brmsformula]{brmsformula}} how to obtain prior
...
#'   \code{\link[brms::stanvar]{stanvar}} to define additional variables for use in
...
#'   \code{brmsthreads} object created by \code{\link[brms::threading]{threading}}. Within-chain
...
#'   \code{\link[brms::opencl]{opencl}}. Can be set globally for the current \R session via
...
#'   such as \code{\link[brms::bridge_sampler]{bridge_sampler}} will not be available. Can be
...
#'   \code{\link[brms::brmsfit_needs_refit]{brmsfit_needs_refit}} to see why refit is deemed necessary.
