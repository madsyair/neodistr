...
To keep the vignette lightweight, we use pre-computed results for the examples. The initial examples demonstrate the workflow using pre-computed data and results, while subsequent sections show the code needed to run the models interactively.
...
df <- data.frame(y)
# R.utils::saveObject(y, file.path(precomp_dir, "fossep_data.rds"),format="rds") # Save pre-computed data
...
# Choose priors
prior <- c(
  set_prior("lognormal(0.5,0.5)", class = "alpha"),
  set_prior("lognormal(0.5,0.5)", class = "beta"),
...
# R.utils::saveObject(ppc_plot_fossep, file.path(precomp_dir, "ppc_plot_fossep.rds"),format="rds") # Save pre-computed plot file.path(precomp_dir, "fossep_data.rds")
...
# R.utils::saveObject(fit_fossep_bnrm, file.path(precomp_dir, "fit_fossep_bnrm.rds"),format="rds") # Save pre-computed fit
```
...
```{r show_fossep_results, eval=FALSE}
  ...
  # R.utils::saveObject(pp_plot, file.path(precomp_dir, "pp_plot_fossep.rds"),format="rds") # Save pre-computed plot
  print(pp_plot)
...
set.seed(123)
x <- runif(100)
e <- rjsep(100, 0, 0.1, 0.8,2)  # Simulated errors from JSEP distribution
y <- 0.5 + 0.8*x + e
xy<-cbind(y,x)
...
# R.utils::saveObject(xy, file.path(precomp_dir,"jsep_data.rds"),format="rds") # Save pre-computed data
```
...
prior_reg <- c(
  ...
  set_prior("lognormal(0,0.5)", class = "beta"),
  set_prior("normal(0,1)", class = "sigma"),
...
# R.utils::saveObject(ppc_plot_jsep, file.path(precomp_dir,"ppc_plot_jsep.rds"),format="rds") # Save pre-computed plot
print(ppc_plot_jsep)
# Show prior predictive check plot
```
...
# R.utils::saveObject(fit_jsep_bnrm, file.path(precomp_dir, "fit_jsep_bnrm.rds"),format="rds") # Save pre-computed fit
```
...
# Convergence diagnostics
 ...
 # R.utils::saveObject(pp_plot, file.path(precomp_dir,"pp_plot_jsep.rds"),format="rds") # Save pre-computed plot

  # Model evaluation
  loo_result <- loo(fit_jsep_bnrm, moment_match = TRUE)
  # R.utils::saveObject(loo_result, file.path(precomp_dir,"loo_result_jsep.rds"),format="rds") # Save pre-computed loo result
  print(loo_result)
