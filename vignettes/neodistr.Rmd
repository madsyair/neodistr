...
ppc_plot_fossep <- pp_check(ppc_fit_fossep, type = "dens_overlay", nsamples = 200) 
...
ppc_plot_fossep <- readRDS("file.path") # Load pre-computed plot
print(ppc_plot_fossep)
# Show prior predictive check plot
```
...
```{r show_fossep_results, eval=FALSE}
  ...
  #saveRDS(pp_plot, file.path(precomp_dir, "ppc_plot_fossep.rds")) # Save pre-computed plot
  print(pp_plot)
  
```

```{r show_fossep_results_pre, eval=TRUE,echo=FALSE,cache=TRUE}
# precomputed results
...
# Read pre-computed plot
  ...
  pp_plot<-readRDS(file.path(precomp_dir,"ppc_plot_fossep.rds")) # Load pre-computed plot
  print(pp_plot)
```
...
```{r ppc_plot_jsep, eval=TRUE,echo=FALSE,message=FALSE,cache=TRUE}


ppc_plot_jsep <- readRDS("data_precomputed/ppc_plot_jsep.rds") # Load pre-computed plot
print(ppc_plot_jsep)
# Show prior predictive check plot
```
...
# Convergence diagnostics
 ...
 #saveRDS(pp_plot, file.path(precomp_dir,"ppc_plot_jsep.rds")) # Save pre-computed plot
...
```{r show_jsep_results_pre, eval=TRUE, echo=FALSE,cache=TRUE}
...
# Convergence diagnostics
  ...
  print(trace_plot)
  # Posterior predictive check
  pp_plot <- readRDS(file.path(precomp_dir,"ppc_plot_jsep.rds")) # Load pre-computed 
  print(pp_plot)
