...
#y <- rfossep(100, 0, 1, 2, 2)  # Simulated data from fossep distribution
#df <- data.frame(y)
y<-readRDS("data_precomputed/fossep_data.rds") # Load pre-computed data
df <- data.frame(y)
# Basic exploration
summary(y)
hist(y, main = "Simulated FOSSEP Data", xlab = "Y", col = "skyblue", breaks = 20)
...
ppc_plot_fossep <- readRDS("data_precomputed/ppc_plot_fossep.rds") # Load pre-computed plot
print(ppc_plot_fossep)
# Show prior predictive check plot
```
...
# precomputed results
  fit_fossep_bnrm <- readRDS("data_precomputed/fit_fossep_bnrm.rds") # Load pre-computed fit
  summary(fit_fossep_bnrm)
...
```{r,eval=TRUE, echo=FALSE}
xy <- readRDS("data_precomputed/jsep_data.rds") # Load pre-computed data
...
fit_jsep_bnrm <- readRDS("data_precomputed/fit_jsep_bnrm.rds") # Load pre-computed fit  
